...
const catalogShowBtn = document.querySelector('.catalog__btn-show');

catalogShowBtn.addEventListener("click", ()=>{
    if (catalog.classList.contains("catalog-show")) {
        catalogShowBtn.innerHTML = "раскрыть каталог";
        catalogShowBtn.style.bottom = "20px";
        catalog.classList.toggle('catalog-show');
    } else {
        catalog.classList.toggle('catalog-show');
        catalogShowBtn.innerHTML = "свернуть каталог";
        catalogShowBtn.style.bottom = "10px";
    }
});
//About
const aboutWrap = document.querySelector('.about__p-wrap');
const aboutWrapBtn = document.querySelector(".about__btn-more");

aboutWrapBtn.addEventListener("click", ()=>{
    aboutWrap.classList.toggle("about__p-wrap-active");
    if (aboutWrapBtn.innerText === "свернуть") {
        aboutWrapBtn.innerText = "читать больше"
    } else {
        aboutWrapBtn.innerText = "свернуть"
    }
})
